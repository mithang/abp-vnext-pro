...
import { DataListProps, PaginationParams, PaginatedResponse } from '../../types';

interface DataListComponentProps<T = any> extends DataListProps<T> {
  navigation?: any;
...
function DataList<T = any>({
  ...
  const fetch = (skip: number = 0, isRefreshingActive: boolean = true): Promise<void> => {
    if (isRefreshingActive) setLoading(true);
    return fetchFn({ filter, maxResultCount, skipCount: skip } as PaginationParams)
      .then(({ items, totalCount: total }: PaginatedResponse<T>) => {
        setTotalCount(total);
  ...
  useEffect(() => {
    ...
    const debouncedSearch = debounce(searchFetch, debounceTime);
    debouncedSearch();
  }, [filter]);
    ...
    <Center>
      ...
      >
        ...
        <FlatList
          ...
          renderItem={(info: any) => (
            <>
              {render(info.item, info.index)}
              {info.index + 1 === skipCount + maxResultCount &&
...
const Forwarded = forwardRef<any, any>((props: any, ref: any) => (
